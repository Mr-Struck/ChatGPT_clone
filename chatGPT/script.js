...
let userText = null;
const apiKey = "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",
...
const getChatResponse = async (incomingChatDiv) => {
  ...
  try {
    const response = await (await fetch(API_URL, requesstOptions)).json();
    pElement.textContent = response.choices[0].text.trim();
  } catch (error) {
    pElement.classList.add("error");
    pElement.textContent =
      "Oops! Something went wrong while retreiving the response. Please try again";
  }
